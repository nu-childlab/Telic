...
initprint = 0;
%initprintsubj = 1;
if ~(exist('TELICYData.csv', 'file') == 2)
    initprint = 1;
end
%if ~(exist([subj '.csv'], 'file') == 2)
%    initprintsubj = 1;
%end
dataFile = fopen('TELICYData.csv', 'a');
subjFile = fopen(sprintf([subj '.csv']),'a');
if initprint
    fprintf(dataFile, 'subj,time,cond,break,loops,response\n');
end
%if initprintsubj
fprintf(subjFile, 'subj,time,cond,break,loops,response,presorder\n');
%end

lineFormat = '%s,%6.2f,%s,%s,%d,%s,%d\n';
...
% else %if cond is 'e'
    ...
    for x = 1:numel(pairs)
        ...
        WaitSecs(pauseTime);
        [response, time] = getResponse(window, screenXpixels, screenYpixels, textsize, cond);
        fprintf(dataFile, lineFormat, subj, time*1000, cond, breakType, loop, response, presorder);
        fprintf(subjFile, lineFormat, subj, time*1000, cond, breakType, loop, response, presorder);
        vbl = Screen('Flip', window);
...
function [] = instructions(window, screenXpixels, screenYpixels, textsize, cond, textspace)
    ...
    DrawFormattedText(window, ['Press ' quote 'f' quote ' if you prefer \n'...
        'the sentence on the left.'],screenXpixels/3-175, 'center', textcolor, 70, 0, 0, 1.5);
    DrawFormattedText(window, ['Press ' quote 'j' quote ' if you prefer \n'...
        'the sentence on the right.'],2*screenXpixels/3-150, 'center', textcolor, 70, 0, 0, 1.5);
    Screen('TextSize',window,textsize);
...
function [response, time] = getResponse(window, screenXpixels, screenYpixels, textsize, cond)
    ...
    optf = 'The star GLEEBED in \nthe middle of the screen.';
    optj = 'The star GLEEBED to \nthe middle of the screen.';
   % end

    DrawFormattedText(window, question, 'center', screenYpixels/3, textcolor, 70);
    
    Screen('TextSize',window,textsize + 4);
    DrawFormattedText(window, optf, screenXpixels/3-175, 'center', textcolor, 70, 0, 0, 1.5);
    DrawFormattedText(window, optj, 2*screenXpixels/3-150, 'center', textcolor, 70, 0, 0, 1.5);
    
    Screen('TextSize',window,textsize);
    DrawFormattedText(window, ['Press ' quote 'f' quote],...
        screenXpixels/3-55, screenYpixels/2+40, textcolor, 70);
    DrawFormattedText(window, ['Press ' quote 'j' quote],...
        2*screenXpixels/3-55, screenYpixels/2+40, textcolor, 70);
